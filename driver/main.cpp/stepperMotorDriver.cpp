...
void MotorDriver::toPosition(float mm, unsigned int feedrate) {
  long targetPulse = mm * STEP_PER_MM;
  long dPulse = targetPulse - ABS_POS_PULSE;
  if (dPulse > 0) {
    forward(dPulse, feedrate);
  } else if (dPulse < 0) {
    backward(-dPulse, feedrate);
  }
  Serial.print("dPulse:  ");

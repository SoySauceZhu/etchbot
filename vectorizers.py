"""
Author: Every-Flavor-Robotics
Project Name: etchbot
URL: https://github.com/Every-Flavor-Robotics/etchbot
...
class PotraceVectorizer(Vectorizer):
    ...
    def process(self, image_path: Path, output_path: Path) -> None:
        ...
        with open(output_path, "w") as f:
            f.write(
                f"""<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="{
                    image.width}" height="{image.height}" viewBox="0 0 {image.width} {image.height}">"""
            )
            parts = []
            for curve in path:
                ...
                parts.append("z")
            f.write(
                f'<path stroke="#000000" fill="None" fill-rule="evenodd" d="{
                    "".join(parts)}"/>'
            )
            f.write("</svg>")
